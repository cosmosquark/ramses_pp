	...
	for icpu in cpu_node_list[myrank]:
		...
		if CELL_LEVEL_HISTOGRAM:
			for ilvl in range(lvlmax):
				...
				if leaf_cells_level.has_key(ilvl):
	...
	if myrank == 0:
		for i in range(1, MPI_process_number):
			...
			if CELL_LEVEL_HISTOGRAM:
				...
				for ilvl in leaf_cells_level_I.keys():
					if leaf_cells_level.has_key(ilvl):
		...
		if CELL_LEVEL_HISTOGRAM:
			...
			pos = arange(len(leaf_cells_level.keys()))
			width = 1.0     # gives histogram aspect to the bar diagram
			import matplotlib.pyplot as plt
			ax = plt.axes()
			ax.set_xticks(pos + (width / 2))
			ax.set_xticklabels(leaf_cells_level.keys())
			plt.bar(pos, leaf_cells_level.values(), width)
#			plt.show()
