...
class RegionFinderTab(wx.Panel):#{{{
	...
	label_string = {"los": "line-of-sight view",\
					"u": "u view",\
					"v": "-v view"}
	...
	def __init__(self, parent, model, controller):#{{{
		...
		self.axis_ids = {"los": WID.RF_TAB_LOS_WIN, \
					"u":   WID.RF_TAB_U_WIN, \
					"v":   WID.RF_TAB_V_WIN}
		wx.Panel.__init__(self, parent, WID.RF_TAB)
		self.model = model
	...
	def RefreshOutputTime(self):#{{{
		try: #self.model.ramses_dir != None:
	...
	def IsUVWNoneSet(self):#{{{
		return ((not AxisWindow.is_set_u)*\
				(not AxisWindow.is_set_v)*\
				(not AxisWindow.is_set_w))
	#}}}
	#}}}
#}}}
