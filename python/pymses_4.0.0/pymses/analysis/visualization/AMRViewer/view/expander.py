...
class ParametersExpander(Expander):#{{{
	def __init__(self, parent, model, controller):
		WID = wid()
		Expander.__init__(self, parent, WID.EXPANDER_PARAMETERS, "Parameters", model, controller, is_collapsed=True)
	...
	def make_content(self, parent):#{{{
		...
		self.listregionFinderMapType = ["density","temperature","velocity","Sigma","dm_particles", "stars_particles"]
		...
		self.Multiprocessing_cb.SetValue(str(misc.NUMBER_OF_PROCESSES_LIMIT))
		self.Multiprocessing_cb.SetToolTip(wx.ToolTip("Limit the number of process created with multiprocessing"))
		self.MultiprocessingBtn = wx.lib.buttons.GenBitmapToggleButton(p_two, WID.ANY, wx.Bitmap(get_icon("memory.png")))
		self.MultiprocessingBtn.SetToolTip(wx.ToolTip("Use multiprocessing to draw the 3 region finder map in parallel, USE ONLY IF YOU HAVE ENOUGH RAM MEMORY"))
		...
		self.rememberSomeDataBtn = wx.lib.buttons.GenBitmapToggleButton(p_three, WID.ANY, wx.Bitmap(get_icon("memory.png")))
		self.rememberSomeDataBtn.SetToolTip(wx.ToolTip("Remember some data in cache, USE ONLY IF YOU HAVE ENOUGH RAM MEMORY, and click here again to free all memory"))
		...
		self.random_shiftCheckBox.SetToolTip(wx.ToolTip("Add a random shift to point positions to avoid grid effect on FFT splatting images"))
		...
		self.list_cmap = ["jet", "jet_black","Greys_r","Reds","Greens","Blues","Reds_r","Greens_r","Blues_r","Spectral","RdBu", "RdBu_r"]
		self.cmap_cb.SetItems(self.list_cmap)
		self.cmap_cb.SetValue("jet")
		self.cmap_cb.SetToolTip(wx.ToolTip("Change the colormap used"))
		self.AdaptiveGaussianBlurBtn = wx.lib.buttons.GenBitmapToggleButton(p_four, WID.ANY, wx.Bitmap(get_icon("gaussian_blur.png")))
		self.AdaptiveGaussianBlurBtn.SetToolTip(wx.ToolTip("Experimental : compute local image resolution and apply an adaptive gaussian blur"))
		...
		self.ThreadCheckBox.SetToolTip(wx.ToolTip("Use a separated from GUI thread : don't use this if the GUI crash !"))
		self.insizer.Add(self.ThreadCheckBox, 0, wx.RIGHT | wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT, 5)
		
		misc.init_OpenCl()
		self.model.use_openCL = False
		if misc.OpenCL_initialized:
			# special GUI button for OpenCL
			p_five = wx.Panel(parent, WID.ANY)
			bs = wx.BoxSizer(wx.HORIZONTAL)
			p_five.SetSizer(bs)
			self.openCLCheckBox = wx.CheckBox(p_five, -1, 'Use OpenCL device', (40, 100))
			self.openCLCheckBox.SetToolTip(wx.ToolTip("Use OpenCL if possible: works only with ray tracing with fully loaded octreedatasource yet"))
			self.openCL_cb = wx.ComboBox(p_five, WID.ANY, size=(50,-1), style=wx.CB_READONLY)
			self.openCL_list = ["0","1","2"]
			self.openCL_cb.SetItems(self.openCL_list)
			self.openCL_cb.SetValue("0")
			bs.Add(self.openCLCheckBox, 0, wx.RIGHT | wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT, 5)
			bs.Add(self.openCL_cb, 0, wx.RIGHT | wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_RIGHT, 5)
			self.insizer.Add(p_five, 0, wx.ALIGN_LEFT)
			# Bind the button
			self.openCLCheckBox.Bind(wx.EVT_CHECKBOX, self.update_openCLCheckBox)
			self.openCL_cb.Bind(wx.EVT_COMBOBOX, self.update_openCL_cb)
	...
	def update_openCLCheckBox(self, event):
		self.model.use_openCL = self.openCLCheckBox.GetValue()
		print "use_openCL =", self.model.use_openCL
		self.controller.SaveConfigInCSV()
	
	def update_openCL_cb(self, event):
		misc.init_OpenCl(event.GetSelection())
		if not self.model.use_openCL:
			self.openCLCheckBox.SetValue(True)
			self.update_openCLCheckBox(event)
		self.controller.SaveConfigInCSV()
	...
	def updateMultiprocessingLimit(self, event):
		misc.NUMBER_OF_PROCESSES_LIMIT =  int(self.listMultiprocessing_cb[event.GetSelection()])
		self.controller.SaveConfigInCSV()
...
class LineOfSightExpander(Expander):#{{{
	...
	def make_content(self, parent):#{{{
		...
		self.autorefreshCheckBox.SetToolTip(wx.ToolTip("Automatically update images when the mouse is released on 3D-view"))
		...
		self.buildPreviewCubeCheckBox.SetToolTip(wx.ToolTip("Use amr2cube to create a low resolution preview for 3D-view"))
		self.controller.PreviewCube = False
		self.insizer.Add(self.buildPreviewCubeCheckBox, 0, wx.LEFT, 0)
		
		self.galaxyAxisBtn = wx.Button(parent, -1, 'Find galaxy axis', (20, 100))
		self.galaxyAxisBtn.SetToolTip(wx.ToolTip("If there is a galaxy in the center, look for the perpendicular axis"))
		self.insizer.Add(self.galaxyAxisBtn, 0, wx.CENTER, 0)
		
		self.losBtn = wx.Button(parent, -1, 'Angular momentum', (20, 70))
		self.losBtn.SetToolTip(wx.ToolTip("Adjusts the line of sight along the angular momentum of the gas inside the camera box"))
		self.insizer.Add(self.losBtn, 0, wx.CENTER, 0)
		
		self.CenterBtn = wx.Button(parent, -1, 'Mass center', (20, 30))
		self.CenterBtn.SetToolTip(wx.ToolTip("Find the center of mass in the current camera box"))
		self.insizer.Add(self.CenterBtn, 0, wx.CENTER, 0)
		...
		self.galaxyAxisBtn.Bind(wx.EVT_BUTTON, self.find_galaxy_axis)
		self.CenterBtn.Bind(wx.EVT_BUTTON, self.find_center)
		self.losBtn.Bind(wx.EVT_BUTTON, self.find_los)
	#}}}
	...
	def RefreshAxes3D(self):#{{{
		...
		if self.controller.PreviewCube:
			...
			map = ray_trace_cartesian(self.model.preview_cube, ray_origins, ray_vectors, ray_lengths, cam.get_bounding_box(), mms)
	...
	def updatePreviewCube(self, event):
		...
		else : self.a3d.SetPreviewImage(None)
		self.RefreshAxes3D()
	
	def find_galaxy_axis(self, evt):
		if self.controller.use_separated_thread:
			"""call FindGalaxyAxis() within a dedicated thread"""
			class WorkerThread(Thread):
				def __init__(self, parent_thread):
					Thread.__init__(self)
					self.parent_thread = parent_thread
				def run(self):
					self.parent_thread.model.FindGalaxyAxis(self.parent_thread)
					self.parent_thread.controller.UpdateImages()
			WorkerThread(self).start()
		else:
			self.model.FindGalaxyAxis(self)
			self.controller.UpdateImages()
	
	def find_center(self, evt):
		if self.controller.use_separated_thread:
			"""call FindCenter() within a dedicated thread"""
			class WorkerThread(Thread):
				def __init__(self, parent_thread):
					Thread.__init__(self)
					self.parent_thread = parent_thread
				def run(self):
					self.parent_thread.model.FindCenter(self.parent_thread)
					self.parent_thread.controller.UpdateImages()
			WorkerThread(self).start()
		else:
			self.model.FindCenter(self)
			self.controller.UpdateImages()
	
	def find_los(self, evt):
		if self.controller.use_separated_thread:
			"""call FindLos() within a dedicated thread"""
			class WorkerThread(Thread):
				def __init__(self, parent_thread):
					Thread.__init__(self)
					self.parent_thread = parent_thread
				def run(self):
					self.parent_thread.model.FindLos(self.parent_thread)
					self.parent_thread.controller.UpdateImages()
			WorkerThread(self).start()
		else:
			self.model.FindLos(self)
			self.controller.UpdateImages()
