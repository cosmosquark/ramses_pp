...
class MainWindow(wx.Frame):#{{{
	...
	def __init__(self, parent, title):#{{{
		...
		for tab_index in range(len(WID.TAB_NAME_LIST)):
			...
			self.tabdisplaymenu.Append(WID.TAB_MENU_IDS[tab_name], "%s"%descr," Show/hide tab : '%s'"%descr, wx.ITEM_CHECK)
		...
		toolbar.AddSimpleTool(WID.MENU_SAVECAM, wx.Bitmap(get_icon("camera_save.png")), '', 'Save the camera into a file')
		toolbar.AddSeparator()
		toolbar.AddSimpleTool(WID.TOOL_GLNEMO2, wx.Bitmap(get_icon("glnemo2.png")), '', 'Start glnemo2 on this view box')
		toolbar.AddSeparator()
		toolbar.AddSimpleTool(WID.TOOL_SAVE_IMAGE, wx.Bitmap(get_icon("save.png")), '', 'Save image')
		toolbar.AddSimpleTool(WID.TOOL_BUILD_SOURCE, wx.Bitmap(get_icon("load.png")), '', 'Build a local octree for this area')
		...
		self.stars_age_instensity_dimmingCheckBox = wx.CheckBox(self.toolbar_panel, -1, 'Stars intensity dimming with age', (20, 100))
		self.stars_age_instensity_dimmingCheckBox.SetToolTip(wx.ToolTip("Stars intensity dimming for stars more than 10 Myr old"))
		...
		self.toolbar_sizer.Add(self.stars_age_instensity_dimmingCheckBox, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_LEFT, 5)
			...
			if (tab_menu_id == tid):
				...
				if tab.IsShown():
					...
					if tab_name == "hdf5":
						...
						if self.model.image_computed.has_key(tab_name) and \
