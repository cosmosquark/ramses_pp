...
class RegionFinderController:
	...
	def RegionFinderOnMouse(self, event):#{{{
		...
		if ((event.Moving()+event.Dragging())*((t-self.rf_time) < RegionFinderController.timing_ms)): # Refreshment frequency
			return
		self.rf_time = t
		...
		#}}}
			...
			if ((wh!=0)*rf_view.IsUVWAllSet()*\
					(not rf_view.IsZoomSizeIsSet())):
