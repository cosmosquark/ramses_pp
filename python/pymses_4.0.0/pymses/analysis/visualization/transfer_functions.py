...
class ColorLinesTransferFunction(object):
	...
	def similar(self,tf):#{{{
		...
		"""
		return self.gamma == tf.gamma and self.vlines == tf.vlines \
				 and self.wlines == tf.wlines
	#}}}
		...
		if ((value < self.vmin) or (value > self.vmax)):
			...
			for v, w in zip(self.vlines, self.wlines):
				assert abs(value - v) > (3.*(w+width)), \
						"Error, intersecting gausian distribution : (x0, sigma0) = (%f, %f) and (x1, sigma1) = (%f, %f)"%(value, width, v, w)
