...
!  integer::nvarh=6
  ...
  character(LEN=256)::nomfich,repository,outdir,snapdir,fname,arg,fcool,foutgas,fstar
  logical::ok,ok_cell
  ...
  integer :: nstar,np,ist,istar,jj
  real(kind=8) :: dr_cell,r0,scale_l,scale_d,scale_t,ageu,time_simu,dzz,mp,Msol,dtsfr,amass,dd
  real(KIND=8),dimension(:,:),allocatable::xstar,vstar
  real(KIND=8),dimension(:)  ,allocatable::mstar,astar,zstar,sfr
  integer,dimension(:)  ,allocatable::idstar,itmp
#ifdef CHEMO
  real(kind=8),dimension(:)  ,allocatable::mstar0,tstar
#endif
  ...
  nst2 = 0
  xlim = 200.  !! in kpc
  dtsfr = 50.  !! dtsfr = in 10^6 yrs units
  ...
  !!======================================================================================
     if(dtsfr>0.)then
        ...
        read(13) nstar
        print *,'nstar =',nstar,scale_l
        allocate(xstar(nstar,3),vstar(nstar,3),astar(nstar),mstar(nstar),zstar(nstar),idstar(nstar))
#ifdef CHEMO
        allocate(mstar0(nstar),tstar(nstar))
#endif
...
#ifdef CHEMO
        read(13) mstar0(:nstar)
        read(13) tstar(:nstar)
#endif
        close(13)
        print *,ageu,time_simu,maxval(astar(:nstar))

        xstar(:nstar,1)=xstar(:nstar,1)+0.5d0
        xstar(:nstar,2)=xstar(:nstar,2)+0.5d0
        xstar(:nstar,3)=xstar(:nstar,3)+0.5d0
        do j=1,3
           dx=cm(j)-0.5d0
           xstar(:nstar,j)=xstar(:nstar,j)+dx 
           where (xstar(:nstar,j) < 0.) xstar(:nstar,j)=xstar(:nstar,j)+1.d0
           where (xstar(:nstar,j) > 1.) xstar(:nstar,j)=xstar(:nstar,j)-1.d0
        enddo
        ...
        ii=0
        dtsfr=dtsfr*1.d6
        print *,time_simu-dtsfr/1.e9
        do j=1,nstar
           if(astar(j)<time_simu-dtsfr/1.d9)cycle
           ! x0=abs(xstar(j,1)-cm(1))
           ! y0=abs(xstar(j,2)-cm(2))
           ! z0=abs(xstar(j,3)-cm(3))
           ! dd=sqrt(x0*x0+y0*y0+z0*z0)
           ! if(dd>rvir)cycle                         
           ii=ii+1
           itmp(ii)=j
        enddo
        np=ii
        print *,'nb of young stars ',np,dtsfr        
        Msol=1.9891d+33
        amass=scale_d*scale_l**3/Msol
        dtsfr=amass/dtsfr
     endif
  !!======================================================================================
     ...
     if(dtsfr>0.)then
        ...
        do ii=1,nleaf
           ...
           do ist=1,np
              istar=itmp(ist)
              if(istar==0)cycle
              dxx=cell.pos(ii,1)-xstar(istar,1)
              dyy=cell.pos(ii,2)-xstar(istar,2)
              dzz=cell.pos(ii,3)-xstar(istar,3)
              dr_cell=MAX(ABS(dxx),ABS(dyy),ABS(dzz))
              if(dr_cell .le. dx/2.d0)then
                 mp=mstar(istar)
#ifdef CHEMO
                 mp=mstar0(istar)
#endif
        ...
        print *,'jj ',jj
        print *,'dtsfr ',dtsfr
        print *,'sfr ',maxval(sfr(:nleaf))*amass,maxval(sfr(:nleaf))*dtsfr
        deallocate(xstar,vstar,astar,mstar,zstar,idstar,itmp)
#ifdef CHEMO
        deallocate(mstar0,tstar)
#endif
     endif
...
#endif
     if(dtsfr>0.)then
        write(13) dtsfr                                       ! coeff to convert sfr from code units to Msol.yr-1
        write(13) (sfr(i),i=1,nleaf)                          ! mass of young stars in gas cells, in code units
     endif
  ...
  subroutine read_params
    ...
    do i=1,n,2
       ...
       case ('-xli')
          read(arg,*) xlim
       case ('-sfr')
          read(arg,*) dtsfr  !if 0, no SFR calculation
       case default
