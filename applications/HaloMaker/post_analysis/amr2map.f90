program amr2map
  ...
  !--------------------------------------------------------------------------
  use ramses_info                                                      ! joki
#ifdef JEJE 
  use jeje
#endif
  implicit none
  integer::n,i,j,k,twotondim,ncoarse,type=0,domax=0,doinverse=0,doneutral=0
  integer::ivar,nvar,ncpuh,lmax=0
  ...
  character(LEN=128)::nomfich,outfich,filetype='bin',repository
  logical::ok,ok_part,ok_cell,do_max,do_inverse,do_neutral
...
#ifdef SMOOTH
  logical      :: smoothing
  integer      :: iimin,jjmin,iimax,jjmax,distance,gxmin,gxmax,gymin,gymax,ig,jg,id,jd,ii,jj
  real(kind=8) :: sigma=1.0d0
  real(kind=8) :: sigma2,kernel,norm_kernel,fac,rr2,gg,gd,dg,dd,xpos,ypos
#endif
  ...
  scale_T2   = mH/kB * (real(scale_l,kind=8)/real(scale_t,kind=8))**2

#ifdef JEJE 
  ipos=INDEX(repository,'output_')
  repository_jeje = trim(repository(1:ipos-1))
  read(repository(ipos+7:ipos+13),*) output_nb
  call init_cooling
#endif
...
!  read(10,'("ordering type=",A80)'),ordering
  ...
  do_inverse=.false. !joki
  if(doinverse==1)do_inverse=.true.
  do_neutral=.false. !joki
  if(doneutral==1)do_neutral=.true.
  zzmax=1.0
  zzmin=0.0
  if(ndim>2)then
  if (proj=='x')then
  ...
  do ilevel=1,lmax
     ...
     kmin=int(zzmin*dble(ny_full))+1 !joki
     kmax=int(zzmax*dble(ny_full))+1 !joki
     ...
     do ilevel=1,lmax
        ...
        if(ngrida>0)then
           ...
           do ind=1,twotondim
              ...
              rho = var(:,ind,1)
#ifdef JEJE
              call ramses2real(ngrida,var(:,ind,5),var(:,ind,1),var(:,ind,2),var(:,ind,3),var(:,ind,4))
#endif
!              rho = var(:,ind,1)
              select case (type)
              case (-1)
...
!                 map = var(:,ind,5)
              case (5) ! T/mu(K)-MW
                 map = var(:,ind,5)*scale_T2
#ifdef JEJE
                       ! T(K)-MW, polytrop-corrected                
                 map = var(:,ind,5)*rho
#endif
              case (6) ! Metallicity-MW
!                 map = var(:,ind,6)*rho
!                      Metallicity-MW, in solar units
                 map = var(:,ind,6)*rho/0.019
                 metmax=max(metmax,maxval(var(:,ind,6)))
              case (-7)
!                 map = 0.5*(var(:,ind,1)**2+var(:,ind,2)**2+var(:,ind,3)**2)
              case default                                                      !joki
                 map = var(:,ind,type)
                 if(.not. do_max) map=map*var(:,ind,1)                          !joki
              end select                                                       

              if( do_inverse ) map = 1.d0/map
              if( do_neutral) map = 1.d0-map
              ...
              do i=1,ngrida
                 ...
                 if(ok_cell)then
                    ...
                    iz=int(x(i,kdim)*dble(ny_full))+1 !joki
                    if(ndim==3)then
                    ...
                    if(    ix>=grid(ilevel)%imin.and.&
                         ...
                         & iz>=grid(ilevel)%kmin.and.&             !joki
                         & ix<=grid(ilevel)%imax.and.&
                         & iy<=grid(ilevel)%jmax.and.&
                         & iz<=grid(ilevel)%kmax)then              !joki
                       if(do_max)then
...
#ifdef SMOOTH
              if(ilevel .ge. levelmin)then
                 sigma2=(sigma*0.5**ilevel)**2
                 iimin=int(xxmin*dble(ndom))+1
                 jjmin=int(yymin*dble(ndom))+1
                 iimax=int(xxmax*dble(ndom))
                 jjmax=int(yymax*dble(ndom))
                 norm_kernel=0d0
                 gxmin=MAX(iimin,i-distance)
                 gxmax=MIN(iimax,i+distance)
                 gymin=MAX(jjmin,j-distance)
                 gymax=MIN(jjmax,j+distance)
                 do ii=gxmin,gxmax,1
                    do jj=gymin,gymax,1
                       xpos=((dble(ii)-0.5)/2.0**ilevel)
                       ypos=((dble(jj)-0.5)/2.0**ilevel)
                       rr2=(xpos-xmin)**2.0 + (ypos-ymin)**2.0
                       norm_kernel=norm_kernel+exp( - rr2 / (2.0*sigma2) )
                    enddo
                 enddo
                 do ii=gxmin,gxmax,1
                    do jj=gymin,gymax,1
                       ...
                       ypos=((dble(jj)-0.5)/2.0**ilevel)
                       rr2=(xpos-xmin)**2.0 + (ypos-ymin)**2.0
                       kernel=exp( - rr2 / (2.0*sigma2) )
                       grid(lmax)%map(ix,iy)=grid(lmax)%map(ix,iy) + &
                            & grid(ilevel)%map(ii,jj)*kernel/norm_kernel
                       grid(lmax)%rho(ix,iy)=grid(lmax)%rho(ix,iy) + &
                            & grid(ilevel)%rho(ii,jj)*kernel/norm_kernel
                    enddo
                 enddo
              endif
#else
  ...
  if (filetype=='bin')then
     ...
     if(nx_sample==0)then
        ...
        write(*,*)'Min and Max=',minval(toto),maxval(toto)
        write(20)toto
  ...
  subroutine read_params
    ...
    do i = 1,n,2
       ...
       case ('-max')
          read (arg,*) domax
       case ('-inv')              !joki
          read (arg,*) doinverse  !joki
       case ('-neu')              !joki
          read (arg,*) doneutral  !joki
       case default
