...
!=======================================================================
  ...
  real(kind=4)    :: maxmasssub,mass_acc
  ! jeje 
...
!=======================================================================
  ...
  real(kind=4)      :: mass_acc,masssubmax
...
!=======================================================================
  ...
  integer(kind=4)                     :: ipar
  real(kind=4), dimension(0:nvoisins) :: dist2
...
!=======================================================================
  ...
  real(kind=4)                 :: posfin(3)
  real(kind=4)                 :: densmaxgroup
  ...
  real(kind=4),  allocatable   :: densmaxg(:)
...
!=======================================================================
  ...
  integer(kind=4) :: i
  real(kind=4)  :: density1,density2,rho_sad12
  logical :: exist
  logical, allocatable :: touch(:)
  integer, allocatable :: listg(:)
  real(kind=4),  allocatable :: rho_sad(:)  
...
!=======================================================================
  ...
  implicit none
  real(kind=4)          :: dist2(0:nvoisins)
  integer(kind=4)       :: iparnei(nvoisins)
  real(kind=4)          :: r,unsr,contrib
  real(kind=4),external :: spline
...
!=======================================================================
  ...
  real(kind=4)    :: poshere(3),dist2(0:nvoisins)
  real(kind=4)    :: dx,dy,dz,distance2,sc
  integer(kind=4) :: idist,inc
  integer(kind=4) :: icellid_out
  real(kind=4)    :: discell2(0:8)
  integer(kind=4) :: iparnei(nvoisins)
  integer(kind=4) :: icid(8)

  integer(kind=4) :: i,nbodies_pos_this_node
  real(kind=4)    :: distance2p
  ...
  do while (icell_identity.ne.0)
     ...
     dx=max(0.,min(dx,real(xlong,4)-dx)-sc)
     dy=abs(pos_cell(2,icell_identity)-poshere(2))
     dy=max(0.,min(dy,real(ylong,4)-dy)-sc)
     dz=abs(pos_cell(3,icell_identity)-poshere(3))
     dz=max(0.,min(dz,real(zlong,4)-dz)-sc)
     ...
     if (firstchild(icellid_out) < 0) then
        if (discell2(ic).lt.dist2(nvoisins)) then
           ...
           do i=nbodies_pos_this_node+1,nbodies_pos_this_node+mass_cell(icellid_out)
              ...
              dx=max(0.,min(dx,real(xlong,4)-dx))
              dy=abs(pos(iparcell,2)-poshere(2))
              dy=max(0.,min(dy,real(ylong,4)-dy))
              dz=abs(pos(iparcell,3)-poshere(3))
              dz=max(0.,min(dz,real(zlong,4)-dz))
...
!=======================================================================
   ...
   real(kind=4)              :: xtest(3),pos_this_node_out(3)
   ...
   real(kind=4), allocatable :: size_cell_tmp(:),pos_cell_tmp(:,:)
...
!=======================================================================
  ...
  implicit none
  real(kind=4), parameter :: accurac=1.e-6

  integer, allocatable, dimension(:) :: idgene
  real(kind=4),allocatable, dimension(:) :: tmp
  real(kind=4),allocatable, dimension(:,:) :: possav
  logical,allocatable,dimension(:) :: move
  real(kind=4) :: xref,tolerance,tolerance2,phi,costeta,teta,sinteta
  real(kind=4) :: ran2
