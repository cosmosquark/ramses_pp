...
!*************************************************************************
  ...
  ! distance.
    ...
    integer(kind=4) :: i
    real(kind=4)    :: deldrg,xw,xw2,xw3,xw4,tiny,one,two
...
!*************************************************************************
  subroutine init_cosmo()
    ...
    ! jeje : init sub-box splitting variables (default is do the whole box)
...
#ifdef RECOMPUTE_PROPS
       write(errunit,*) 'STOP: recompute_props not allowed with sub-box option ...'
       stop
#endif
#ifndef BIG_RUN 
...
!*************************************************************************
  ...
  ! computes their properties ...
    ...
    logical(kind=4),allocatable  :: in_the_box(:)
    ! end jeje 
...
!!$    stop
...
#ifdef RECOMPUTE_PROPS
    write(errunit,*) '> reading tree_brick file'
    select case(method)
    case("FOF")
       fsub = .false.
    case("HOP")
    ...
    case("BHM")
       fsub = .true.
    case default
       write(errunit,*) 'CRASHHHHH!'
       stop
    end select
    call read_tree_brick  ! read halos from file and build particle linked list
#else
    ...
    ! jeje : flag main halos which have at least one pad-particle in any of their sub-halos (including main)
    if (subboxFoF) then 
    ...
    if (subboxFoF) write(errunit,*) '> SUBBOX: nb of halos and subhalos in the subbox:',subbox_nh,subbox_ns
...
!!$       end do
    ...
    if (subboxFoF) then 
       write(agor_unit) subbox_nh,subbox_ns
    else
...
!*************************************************************************
  subroutine det_mass(h)
    ...
    h%m = real(masshalo,4)  ! in 10^11 M_sun
...
!***********************************************************************
  ...
  ! compute position of center of mass of halo, and its velocity.
    ...
    h%v%x = real(vcx,4)/h%m
    h%v%y = real(vcy,4)/h%m
    h%v%z = real(vcz,4)/h%m
...
!***********************************************************************
  function interact(i,j)
    ...
    real(kind=4)    :: dist2ij
    real(kind=4)    :: interact,rinveff,r3inveff
    real(kind=4)    :: epstmp,massp2,lbox2
    type (vector)   :: dr
...
!***********************************************************************
  ...
  ! been initialized by initgsoft.
    ...
    real(kind=4)    :: epsp,epsi,drdotdr,sdrdotdr,rinveff,r3inveff,drdeldrg
    real(kind=4)    :: drsm,phsm,epseff,epsinv,dr,tiny,one 
...
!***********************************************************************
  ...
  ! halo (cdm or mbp)
    ...
    real(kind=4)           :: amax,bmax,cmax,v(3,3)
    real(kind=8)           :: tabm2(0:nr-1),tabk2(0:nr-1),tabp2(0:nr-1) !! double
    real(kind=8)           :: srm,srk                                   !! double
    real(kind=4)           :: rmax,dra,drb,drc
    real(kind=4)           :: r_ell,v2,rf
    real(kind=4),parameter :: epsilon = 1.e-2
    type (vector)          :: posp,vt
    type (halo)            :: h
...
!***********************************************************************
  ...
  subroutine det_vir_props(h,v)
    ...
    real(kind=4)              :: rvir,mvir,kvir,pvir,v(3,3)
    real(kind=4)              :: amax,bmax,cmax,avir,bvir,cvir
    real(kind=8)              :: tab_mass(0:ttab-1),tab_ekin(0:ttab-1),tab_epot(0:ttab-1)  !! double
    real(kind=4)              :: virth,virth_old,volmin
    type (halo)               :: h
    ...
    enddo
    avir      = real(i,4)/real(ttab-1,4)*amax
    bvir      = real(i,4)/real(ttab-1,4)*bmax
    cvir      = real(i,4)/real(ttab-1,4)*cmax
    ...
    do i = ttab-1,0,-1
       ...
       if (virth < virth_old) then 
          ...
          avir      = min(avir,real(i,4)/real(ttab-1,4)*amax)
          bvir      = min(bvir,real(i,4)/real(ttab-1,4)*bmax)
          cvir      = min(cvir,real(i,4)/real(ttab-1,4)*cmax)
          rvir      = (avir*bvir*cvir)**(1./3.)
          kvir      = tab_ekin(i) 
          pvir      = tab_epot(i)
          virth_old = virth
    ...
    volmin   = 4./3.*pi*(amax/real(ttab-1,4))*(bmax/real(ttab-1,4))*(cmax/real(ttab-1,4))
    if (virth > 0.20 .or. mvir < vir_overdens*rho_mean*4./3.*pi*avir*bvir*cvir) then
       do ii = ttab-1,1,-1
          ...
          ! ellipsoid shell 
          mvir = vir_overdens * rho_mean * volmin * real(ii,4)**3
          if (tab_mass(ii) >= mvir .and. tab_mass(ii-1) < tab_mass(ttab-1)) exit
       enddo
       mvir   = tab_mass(ii)
       kvir   = tab_ekin(ii)
       pvir   = tab_epot(ii)
       avir   = real(ii,4)/real(ttab-1,4)*amax
       bvir   = real(ii,4)/real(ttab-1,4)*bmax
       cvir   = real(ii,4)/real(ttab-1,4)*cmax
       rvir   = (avir*bvir*cvir)**(1./3.)
    endif
...
#ifdef RECOMPUTE_PROPS 
!!$       if (abs(h%datas%mvir - mvir) > 0.0001) then 
!!$          print*,'mvir has changed !!! '
!!$          print*,h%my_number
!!$          print*,'mismatch in nb of particles : ',abs(h%datas%mvir - mvir)/massp
!!$          print*,'out of : ',h%m/massp
!!$       end if
#endif
...
!***********************************************************************
  function temps_turn_around(x,omm,oml)
    ...
    real(kind=4) :: temps,temps_turn_around,x,omm,oml
...
!***********************************************************************
  function dtda_turn_around(x,omm,oml)
    ...
    real(kind=4) :: x,omm,oml,temp,dtda_turn_around
...
!***********************************************************************
  ...
  ! values of current cosmological parameters 
    ...
    external midpnt
    real(kind=4) :: age0,age1,somme0,somme1,omm,oml
    save age0

    omm  = omega_f
    oml  = omega_lambda_f
    ! age0 is the age of the universe @ the beginning of the simulation (in Gyr)
    call qromo(temps,omm,oml,(af/ai),real(10001.,4),somme0,midpnt) 
...
!***********************************************************************
  subroutine virial()
    ...
    real(kind=4) :: a,b,eta,omega_maxexp,age,reduce,cubic
    ...
    call collapse(age/2.0,omega_maxexp,1.e-6)
...
!***********************************************************************
  subroutine collapse(age0,omm,acc)
    ...
    real(kind=4)            :: age0,omm,acc,oml
    real(kind=4)            :: age,omax,omin,age_f
    real(kind=4), parameter :: omax0=1.e7,omin0=1.e0 
    ...
    do while (abs(age-age0) .gt. acc*age0 .and. omax-omin .gt. acc*omm)
       omm = 0.5*(omax+omin)
       call qromo(temps_turn_around,omm,oml,real(101,4),real(10001,4),age_f,midpnt)
       call qromo(temps_turn_around,omm,oml,real(1,4),real(101,4),age,midpnt)
...
!***********************************************************************
  subroutine det_halo_energies(h)
    ...
    real(kind=4)              :: v2,rf          ! rf is elliptic integral function from numrec
    ...
    if (.not.count_pairs) then 
       ...
       h%ep = real(ped,4) * gravconst / Lbox_pt
    end if
...
!***********************************************************************
  ...
  ! Compute inertial tensor with respect to center of halo (either cdm or mbp)
    ...
    mat(1,1) = real(md(1,1),4)
    mat(1,2) = real(md(1,2),4)
    mat(1,3) = real(md(1,3),4)
    mat(2,1) = real(md(2,1),4)
    mat(2,2) = real(md(2,2),4)
    mat(2,3) = real(md(2,3),4)
    mat(3,1) = real(md(3,1),4)
    mat(3,2) = real(md(3,2),4)
    mat(3,3) = real(md(3,3),4)
...
!***********************************************************************
  ...
  subroutine det_ang_momentum_per_shell(h,amax,bmax,cmax,v)
    ...
    type(halo)      :: h
    type(vector)    :: dr, dv
    real(kind=4)    :: amax,bmax,cmax,v(3,3) ! computed in tab_props_inside
    real(kind=4)    :: dra, drb, drc, r_ell
    ...
    do while(indexp /= -1)
       ...
       if (i_ell > nshells) then 
          ...
          stop
       else if (i_ell == 0) then ! center of shell may be exactly on a particles
          i_ell = 1
       end if
    ...
    ! jeje : if sub-box, re-number on the output
    if (subboxFoF) then 
       if (h%hostsub > 0) then 
          hsub = old2new(h%hostsub)
       else
          hsub = h%hostsub
       end if
       if (h%nextsub > 0) then 
          nextsub = old2new(h%nextsub)
       else
          nextsub = h%nextsub
       end if
       write(agor_unit) old2new(h%my_number), h%p%x, h%p%y, h%p%z, h%v%x, h%v%y, h%v%z, &
            ...
            h%level,old2new(h%hosthalo),hsub,h%nbsub,nextsub
    else
       write(agor_unit) h%my_number, h%p%x, h%p%y, h%p%z, h%v%x, h%v%y, h%v%z, &
            h%datas%rvir, h%datas%mvir, h%m, h%r, h%spin,                      &
            amax, bmax, cmax, v(1,1:3), v(2,1:3), v(3,1:3),                    &
            m(1:nshells), L(1:nshells)%x, L(1:nshells)%y, L(1:nshells)%z,      &
            h%level,h%hosthalo,h%hostsub,h%nbsub,h%nextsub
    end if
!!$
!!$    write(agor_unit) h%my_number, h%p%x, h%p%y, h%p%z, h%v%x, h%v%y, h%v%z, &
!!$         h%datas%rvir, h%datas%mvir, h%m, h%r, h%spin,                      &
