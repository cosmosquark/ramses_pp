...
subroutine qromo(func,omm,oml,a,b,ss,choose)
  ...
  integer(kind=4)           :: j
  real(kind=4),parameter    :: eps=1e-6
  real(kind=4)              :: s(jmaxp),h(jmaxp),a,b,ss,func,dss,omm,oml
  external choose      
...
subroutine polint(xa,ya,n,x,y,dy)
  ...
  real(kind=4)              :: xa(n),ya(n),c(nmax),d(nmax),x,y,dif,dift
  real(kind=4)              :: ho,hpa,den,w,dy
...
subroutine midpnt(func,omm,oml,a,b,s,n)
  ...
  real(kind=4)    ::  func,a,b,s,x,ddel,del,sum,omm,oml
...
subroutine jacobi(a,np,d,v,nrot)
  ...
  real(kind=4)              :: a(np,np),d(np),v(np,np)
  integer(kind=4)           :: i,ip,iq,j
  real(kind=4)              :: c,g,h,s,sm,t,tau,theta,tresh,b(nmax),z(nmax)
...
function rf(x,y,z)
  ...
  real(kind=4)           :: rf,x,y,z
  real(kind=4),parameter :: errtol=.08,tiny=1.5e-38,big=3.e37,third=1./3.,c1=1./24.,c2=.1,c3=3./44.,c4=1./14.
  real(kind=4)           :: alamb,ave,delx,dely,delz,e2,e3,sqrtx,sqrty,sqrtz,xt,yt,zt
...
!     Formulas," 2nd ed., Boca Raton: CRC Press 1992, pp. 8-9.
  ...
  real(kind=4) :: a1,a2,a3,a4,cubic
  real(kind=4) :: a,b,d,r,theta,pi,ar,ai,y,y1,y2,y3,p,q
