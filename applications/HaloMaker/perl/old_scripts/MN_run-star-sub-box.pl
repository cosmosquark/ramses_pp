...
#my $runDir = "/simu1/MN/GalaxyMaker/mario/MSM/";
my $runDir = "/data33/blaizot/MN/GalaxyFinder/MSM_rhot3000/";
...
foreach (@snapList)
  {
    ...
    #my $fileRoot  = "/simu1/MN/outputs/output_$num/";
...
sub write_input_hm {
    ...
    print FILE "omega_b        =  0.045        	! Omega matter at z=0 (ex: 0.3) \n";
    ...
    print FILE "method          =  MSM        	! choose between FOF, HOP, DPM, MSM, MHM (view ReadMe for details). \n";
    ...
    print FILE "nvoisins        =  20   !20     ! parameter for adaptahop (usually 20)\n";
    print FILE "nhop            =  20   !20     ! parameter for adaptahop (usually 20)\n";
    print FILE "rhot            =  3000       	! parameter for adaptahop (80 coreespond to b = 0.2)\n";
    print FILE "fudge           =  4.   !5.     ! parameter for adaptahop (usually 4.) \n";
    print FILE "fudgepsilon     =  0.0 	        ! parameter for adaptahop (usually 0.05)\n";
    print FILE "alphap          =  3.   !1.     ! parameter for adaptahop (usually 1.)\n";
    print FILE "verbose         = .true.     	! verbose parameter for both halo finder\n";
    print FILE "megaverbose     = .true.    	! parameter for adaptahop\n";
    close(FILE);
}
...
sub get_fudge{
    my $i=$_[0];
    # $fue = minimum size of the cells at snapshot 128 = level 15

    my $fue = 1.0;
    for (my $j=0;$j<15;$j++) {$fue = $fue / 2.0 } # compute $fue = 1/2^15 the hard way ...
    
    if ($i < 128) {$fue = $fue * 2;}
    if ($i < 34)  {$fue = $fue * 2;}
    # test fudgepsilon = 2 * min cell size
    $fue = 2 * $fue;
